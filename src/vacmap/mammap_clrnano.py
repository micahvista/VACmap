...
def fastq_q2b(qstring):
    return array.array('B', [ord(c) - 33 for c in qstring])
...
def split_alignment_test(alignment, testseq, rc_testseq, testseq_len, kmersize, contig2seq, contig2start, debug, H = False):#no big cost >30
    ...
    if(alignment[0][2] == 1):
        ...
        while(iloc < len(alignment)):
            ...
            if((nowitem[3] < 15) or (min(readgap, refgap) < min_gap_forcigar)):
                if(iloc +1 != len(alignment)):
                    iloc += 1
                    continue
...
########################
#1121
print('load get_optimal_chain_sortbyreadpos_forSV_inv_test_merged_fine_list:  at 30, p 20, maxop 20,000, hit2work 150, noguide, mapq 0, nr2p, fs')
