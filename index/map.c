	...
	for (i = 0, *n_a = 0; i < n_m; ++i) {
		...
		for (k = 0; k < q->n; ++k) {
			...
			if ((r[k]&1) == (q->q_pos&1)) { // forward strand
				...
				p.y = (uint64_t)q->q_span << 32 | (q->q_pos) >> 1;
				ref_pos = (int64_t)(int32_t)p.x + (int64_t)mi->seq[p.x<<1>>33].offset - mi->k + 1;
			} else{ 
			...
			read_pos = (int64_t)(int32_t)p.y - mi->k + 1;
			s_p->x = ref_pos;
			s_p->y = read_pos;
		}
	}
...
s_mm128_t *mm_map_frag_(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname, int *n_aa)
{
	...
	//aa = collect_seed_hits(b->km, opt, 1000, mi, qname, &mv, qlen_sum, &n_a, &rep_len, &n_mini_pos, &mini_pos);
