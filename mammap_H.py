...
def get_readmap_DP_test(readid, testseq, contig2start, contig2seq, minimap, index2contig, kmersize = 15, local_kmersize = 9, local_maxdiff = 10, refine = True, local_skipcost = 30., golbal_skipcost = (35., 15.),  golbal_maxdiff = (50, 30), check_num = 20, bin_size = 100, hastra = False, debug = False):
    mapq = 60
    setting_kmersize = kmersize
    setting_maxdiff = golbal_maxdiff[1]
    local_skipcost += local_kmersize
...
def get_list_of_readmap(raw_queue, savepath, minimap, contig2seq, hastra):
    ...
    tmp_time = time.time()
    a_list = []
    with pysam.AlignmentFile(savepath+'.bam', "wb", header=header, threads=16) as outf:
        while(True):
            ...
            try:
                while(True):
                    if(psutil.virtual_memory()[2] < 80):
                        ...
                        time.sleep(10)
                onemapinfolist, (alignment_list,raw_alignment_list), one_mapinfo = get_readmap_DP_test(readidandseq[0], readidandseq[1], contig2start, contig2seq, minimap, index2contig, hastra = hastra)
            except:
                onemapinfolist = []
                unmapcountlist.append(readidandseq[0])
                total_r_time += time.time() - r_st
...
def extend_func(raw_alignment_list, readid, mapq, testseq, rc_testseq, testseq_len, setting_kmersize, pos2contig, contig2start, contig2seq, setting_maxdiff, debug = False):
    ...
    if(len(alignment_list) >= 2):
        ...
        while((iloc + 1) < len(alignment_list)):
            ...
            if(refgap < 0):
                iloc += 1
                continue
            if(((testoverlap(alignment_list, iloc) < 50) and (min(readgap, refgap)<100) and (abs(readgap - refgap) < 2000))):
            #if((min(readgap, refgap) < merge_smallgap) and (max(readgap, refgap) < too_large_gap)):
            #if(((min(presize, nowsize) > 500) or ((abs(readgap - refgap) / min(presize, nowsize)) < 0.5)) and (max(readgap, refgap) < 20000)):
                alignment_list[iloc] = List_merge((alignment_list[iloc], alignment_list[iloc + 1]))
                alignment_list.pop(iloc+1)
    ...
    return get_onemapinfolist(new_alignment_list, cigarlist, readid, mapq, testseq_len, contig2start)
@njit
def rebuild_chain_break(contig2start, raw_alignment_list, large_cost, small_alignment = 20, small_dup = -20):
    #rebuild chain break
    #step 1
    #remove anchor cause large cost and small alignment
    small_alignment = 20
...
def testoverlap(alignment_list, iloc):
    ...
    return overlapsize
def split_alignment_test(alignment, testseq, rc_testseq, testseq_len, contig2seq, contig2start, kmersize, debug):#no big cost >30
...
def extend_func(raw_alignment_list, readid, mapq, testseq, rc_testseq, testseq_len, setting_kmersize, pos2contig, contig2start, contig2seq, setting_maxdiff, debug = False):
    ...
    if(debug == True):
        ...
        for line in alignment_list:
            ...
            temprefbias = contig2start[tempcontig]
            print(line[0][0], line[-1][0], line[0][1] - temprefbias, line[-1][1] - temprefbias)
            print(line[0], line[-1], line[0][1] - temprefbias, line[-1][1] - temprefbias)
        ...
        if(onealignment[0][0]>0):
            while(True):
                ...
                if(preitem[2] == 1):
                    ...
                    if(looksize == 0):
                        if(debug): 
                            print('looksize == 0, skiped')
                            print(target_st, target_en, query_st, query_en)
                        break
                    query = testseq[max(query_st - looksize, 0): query_st][::-1]
...
print('Enable high sensitivity mode')