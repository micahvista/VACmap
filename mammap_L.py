...
def get_readloc_set_bin(one_mappos, bin_size):
    ...
    return readloc_set
@njit
def get_optimal_chain_sortbyreadpos_forSV_inv_test_merged_fine(one_mapinfo, kmersize = 15, skipcost = 50., maxdiff = 30, maxgap = 500, magic = 0, maxop = 100000000):
    ...
    for i in range(n):
        S.append(0.)
        P.append(9999999)
        max_scores = one_mapinfo[i][3]+0.
        pre_index = 9999999
        bestnocost_j = -9999999
    ...
    return S, P, g_max_index#, testdict
@njit
...
@njit
def get_localmap_multi_all_forDP_inv_guide(raw_alignment_list, testseq, rc_testseq, contig2start, contig2seq, kmersize, shift = 1):
    
    raw_alignment_list.sort(key = getsecond)
    startandend = [[raw_alignment_list[0][1], raw_alignment_list[0][1]]]
    for item in raw_alignment_list[1:]:
        if((item[1] - startandend[-1][1]) < 2000):
            startandend[-1][1] = item[1]
        else:
    ...
    for item in startandend:
        ...
        if(testcontig != pos2contig(max_ref, contig2start)):
            print('testcontig != pos2contig(max_ref, contig2start)')
            continue
    ...
    one_mapinfo = List([(-1, -1, -1, -1)])
    one_mapinfo.pop(0)
  
            

    raw_alignment_list.sort(key = getfirst)
    readstart = max(0, raw_alignment_list[0][0]-500)
    readend = min(len(testseq)-kmersize+1, raw_alignment_list[-1][0]+500)
    ...
    while(True):
        ...
        if(hash(testseq[iloc: iloc + kmersize]) == hash(rc_testseq[-(iloc + kmersize): -iloc])):
            continue
        biasvalue, biasvalue_1, closest_index, closest_index_1 = findClosest(raw_alignment_list, target = iloc)


        interval = min(biasvalue + biasvalue_1 + 500, 2000)
        upperrefloc =  (raw_alignment_list[closest_index][1] + interval, raw_alignment_list[closest_index_1][1] + interval)
        lowerrefloc =  (raw_alignment_list[closest_index][1] - interval, raw_alignment_list[closest_index_1][1] - interval)
                ...
                if((upperrefloc[0] >= refloc and lowerrefloc[0] <= refloc) or (upperrefloc[1] >= refloc and lowerrefloc[1] <= refloc)):
                    ...
                    if(point in pointdict):
                        ...
                        if((pointdict[point][-1][0] + pointdict[point][-1][3]) >= item[0]):
                            ...
                            if(bouns > 0):
                                if((pointdict[point][-1][3] + bouns < 30)):
    ...
    one_mapinfo.sort(key = getfirst)
  
    

    return True, one_mapinfo
        ...
        if(len(mapinfo) > 1):
            ...
            for item in mapinfo:
                ...
                if(tmpiloc == iloc):
                    continue
                if(item[2] == '+'):
                    nm = compute_NM_tag(query[item[3]: item[4]], contig2seq[item[1]][item[5]: item[6]], item[2])
                else:
                    nm = compute_NM_tag(rc_query[item[3]: item[4]], contig2seq[item[1]][item[5]: item[6]], item[2])
...
@njit
def hit2work(one_mapinfo, index2contig, contig2start, testseq_len, skipcost, maxdiff, maxgap, check_num, c_bias, bin_size, kmersize, overlapprecentage = 0.5):
    def getsecond(x):
        return x[1]
    def getfirst(x):
        return x[0]
    ...
    for one_mapinfo in cluster_list:
        ...
        if((max(readspan, refspan) / min(readspan, refspan)) > 2):
            readspan, refspan = len(get_readloc_set_bin(path, 100)), len(get_refloc_set_bin(path, 100))
            if((max(readspan, refspan) / min(readspan, refspan)) > 2):
                scores, path = get_optimal_chain_sortbyreadpos_forSV_inv_test_merged_fine_list(one_mapinfo[np.argsort(one_mapinfo[:,0])], kmersize = kmersize, skipcost = 1000, maxdiff = maxdiff[0], maxgap = maxgap)
...
@njit
def hit2work(one_mapinfo, index2contig, contig2start, testseq_len, skipcost, maxdiff, maxgap, check_num, c_bias, bin_size, kmersize, overlapprecentage = 0.5, hastra = False):
    def getsecond(x):
        return x[1]
    def getfirst(x):
        return x[0]
    ...
    if(repeat == True):
        ...
        for one_mapinfo in cluster_list:
            ...
            readspan, refspan = len(get_readloc_set_bin(path, 100)), len(get_refloc_set_bin(path, 100))
            if((max(readspan, refspan) / min(readspan, refspan)) > 2):
                scores, path = get_optimal_chain_sortbyreadpos_forSV_inv_test_merged_fine_list_linear(one_mapinfo[np.argsort(one_mapinfo[:,0])], kmersize = kmersize, skipcost = skipcost[0], maxdiff = 200, maxgap = maxgap)
            if(hit == False):
                hit = True
                path_list = List([path])
                scores_list.append(scores)
...
def extend_func(raw_alignment_list, readid, mapq, testseq, rc_testseq, testseq_len, setting_kmersize, pos2contig, contig2start, contig2seq, setting_maxdiff, debug = False):
    ...
    for alignment in alignment_list: 
        tmp_alignment_list, tmp_cigarlist = split_alignment_test(alignment, testseq, rc_testseq, testseq_len, kmersize=setting_kmersize, debug = debug)
...
def get_readmap_DP_test(readid, testseq, contig2start, contig2seq, minimap, index2contig, kmersize = 15, local_kmersize = 9, local_maxdiff = 10, refine = True, local_skipcost = 50., golbal_skipcost = (35., 15.),  golbal_maxdiff = (50, 30), check_num = 20, bin_size = 100, debug = False):
    mapq = 60
    ...
    st = time.time()                            
    mapq, scores, raw_alignment_list = decode_hit(minimap, index2contig, testseq, testseq_len, contig2start, kmersize, skipcost = golbal_skipcost, maxdiff = golbal_maxdiff, maxgap = 2000, check_num = check_num, c_bias = 5000, bin_size = bin_size, overlapprecentage = 0.5)
    ...
    return onemapinfolist, (alignment_list, raw_alignment_list), _
def get_onemapinfolist(new_alignment_list, cigarlist, readid, mapq, testseq_len, contig2start):
    onemapinfolist = []
    iloc = -1
    for alignment in new_alignment_list:
...
def split_alignment_test(alignment, testseq, rc_testseq, testseq_len, kmersize, contig2seq, contig2start, debug):#no big cost >30
            ...
            if(len(target) > 0 and len(query) >0):
                ...
                if(zdroped == True or indel_present == True):
                    ...
                    if(split == True):
                        ...
                        if((((batch[1][0] - batch[0][0]) > 10)) and (((batch[1][0] - batch[0][0]) > merge_smallgap) or indel_present == True)):    
            ...
            if(len(target) > 0 and len(query) >0):
                ...
                if(zdroped == True or indel_present == True):
                    ...
                    if(split == True):
                        ...
                        if(((batch[0][0] - batch[1][0]) > 10) and (((batch[0][0] - batch[1][0]) > merge_smallgap) or indel_present == True)):
        ...
        if(onealignment[0][0]>0):
            while(True):
                ...
                if(preitem[2] == 1):
                    ...
                    #zdroped, max_extend, cigarstring, Score = fast_globalms_align_extend(target, query, match = 2, mismatch = -4, gap_open = gap_open_extend, gap_extend = gap_extend_extend, zdrop = zdrop_value_extend, loc_only = True)
                    cigarstring, zdropedcode, q_e, t_e, tmpdelcount, tmpinscount = mp.k_cigar(target, query, match = 2, mismatch = -4, gap_open_1 = 4, gap_extend_1 = 2, gap_open_2 = 24, gap_extend_2 = 1, bw=bw, zdropvalue=zdrop_value_extend)
...
def extend_func(raw_alignment_list, readid, mapq, testseq, rc_testseq, testseq_len, setting_kmersize, pos2contig, contig2start, contig2seq, setting_maxdiff, debug = False):
    ...
    return get_onemapinfolist(new_alignment_list, cigarlist, readid, mapq, testseq_len, contig2start)
def get_list_of_readmap(readid_testseq_list, savepath, contig2start, contig2seq, minimap, index2contig, header, contig2iloc):
    st = time.time()
    ...
    tmp_time = time.time()
    a_list = []
    for readidandseq in readid_testseq_list:
        #print(iloc, readidandseq[0])
        r_st = time.time()
        try:
            onemapinfolist, (alignment_list,raw_alignment_list), one_mapinfo = get_readmap_DP_test(readidandseq[0], readidandseq[1], contig2start, contig2seq, minimap, index2contig)
        except:
            onemapinfolist = []
        ...
        if(len(onemapinfolist) != 0):
            ...
            unmapcountlist.append(readidandseq[0])
    print('half running time', time.time() - tmp_time, total_r_time, len(a_list))
    if(len(a_list) > 0):
        with pysam.AlignmentFile(savepath+'.bam', "wb", header=header) as outf:
            for a in a_list:
                outf.write(a)

    print(total_r_time, time.time() - st, savepath, 'unmapsize', len(unmapcountlist), )
...
def extend_func(raw_alignment_list, readid, mapq, testseq, rc_testseq, testseq_len, setting_kmersize, pos2contig, contig2start, contig2seq, setting_maxdiff, debug = False):
    ...
    for alignment in alignment_list: 
        ...
        tmp_alignment_list, tmp_cigarlist = split_alignment_test(alignment, testseq, rc_testseq, testseq_len, kmersize=setting_kmersize , contig2seq = contig2seq, contig2start = contig2start, debug = debug)
        if(debug): print(len(tmp_alignment_list), len(tmp_cigarlist))
        iloc = -1
        for alignment in tmp_alignment_list:
            iloc += 1
...
def decode_hit(minimap, index2contig, testseq, testseq_len, contig2start, kmersize, skipcost = (50., 30.), maxdiff = (50, 30), maxgap = 2000, check_num = 20, c_bias = 5000, bin_size = 100, overlapprecentage = 0.5, hastra = True):            
    return hit2work(np.array(minimap.map(testseq)), index2contig, contig2start, testseq_len, skipcost, maxdiff, maxgap, check_num, c_bias, bin_size, kmersize, overlapprecentage, hastra)

def get_readmap_DP_test(readid, testseq, contig2start, contig2seq, minimap, index2contig, kmersize = 15, local_kmersize = 9, local_maxdiff = 10, refine = True, local_skipcost = 50., golbal_skipcost = (35., 15.),  golbal_maxdiff = (50, 30), check_num = 20, bin_size = 100, hastra = False, debug = False):
    mapq = 60
    setting_kmersize = kmersize
    setting_maxdiff = golbal_maxdiff[1]
    local_skipcost += local_kmersize
    ...
    testseq_len = len(testseq)
    st = time.time()                            
    mapq, scores, raw_alignment_list = decode_hit(minimap, index2contig, testseq, testseq_len, contig2start, kmersize, skipcost = golbal_skipcost, maxdiff = golbal_maxdiff, maxgap = 2000, check_num = check_num, c_bias = 5000, bin_size = bin_size, overlapprecentage = 0.5, hastra = hastra)
    ...
    return onemapinfolist, (alignment_list, raw_alignment_list), 1
def iftra(onemapinfolist, hitsize = 200):
    ...
    for nowline in onemapinfolist[1:]:
        ...
        if(min(readgap, refgap) > hitsize):
            return True
    return False
def get_list_of_readmap(raw_queue, savepath, minimap, contig2seq, hastra):
    st = time.time()
    ...
    with pysam.AlignmentFile(savepath+'.bam', "wb", header=header) as outf:
        while(True):
            if(cooked_queue.empty() == True):
                ...
                outf.write(a)
    print('Finish Write')
def get_readmap_DP_test(readid, testseq, contig2start, contig2seq, minimap, index2contig, kmersize = 15, local_kmersize = 9, local_maxdiff = 10, refine = True, local_skipcost = 50., golbal_skipcost = (30., 30.),  golbal_maxdiff = (50, 30), check_num = 20, bin_size = 100, hastra = False, debug = False):
    mapq = 60
    setting_kmersize = kmersize
    setting_maxdiff = golbal_maxdiff[1]
    local_skipcost += local_kmersize
    ...
    st = time.time()
    alignment_list, onemapinfolist = extend_func(List(raw_alignment_list[::-1]), readid, mapq, testseq, rc_testseq, testseq_len, setting_kmersize, pos2contig, contig2start, contig2seq, setting_maxdiff, debug = debug)
    if(debug == True): print('extend_func', time.time() - st)
    if(len(onemapinfolist) > 1 and refine == 'auto'):
        setting_maxdiff = local_maxdiff
        setting_kmersize = local_kmersize
...
#######################
@njit
def get_optimal_chain_sortbyreadpos_forSV_inv_test_merged_fine_list_(one_mapinfo, kmersize = 15, skipcost = 50., maxdiff = 30, maxgap = 500):
...
@njit
def hit2work_(one_mapinfo, index2contig, contig2start, testseq_len, skipcost, maxdiff, maxgap, check_num, c_bias, bin_size, kmersize, overlapprecentage = 0.5, hastra = False):
    def getsecond(x):
        return x[1]
    def getfirst(x):
        return x[0]
    ...
    if(hit == True):
        ...
        return 0, 0., [(0, 0, 0, 0)]
def extend_func_(raw_alignment_list, readid, mapq, testseq, rc_testseq, testseq_len, setting_kmersize, pos2contig, contig2start, contig2seq, setting_maxdiff, debug = False):
    ...
    alignment_list = rebuild_chain_break(contig2start, raw_alignment_list, large_cost = setting_maxdiff, small_alignment = 30, small_dup = -30)
    ...
    return get_onemapinfolist(new_alignment_list, cigarlist, readid, mapq, testseq_len, contig2start)
@njit
def rebuild_chain_break_(contig2start, raw_alignment_list, large_cost, small_alignment = 20, small_dup = -20):
    #rebuild chain break
    #step 1
    #remove anchor cause large cost and small alignment
    small_alignment = 20
    ...
    return alignment_list
def split_alignment_test_(alignment, testseq, rc_testseq, testseq_len, contig2seq, contig2start, kmersize, debug):#no big cost >30
